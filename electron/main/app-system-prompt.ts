...
当用户提出需要查找信息的请求时，**必须先判断查询类型，再选择对应工具**：

| 查询类型 | 示例 | 使用工具 |
|---------|------|---------|
| 学术文献、论文引用 | "XX领域最新论文"、"帮我找相关文献" | research-lookup → 学术数据库 |
| 产品/技术/新闻 | "最新发布的大模型"、"XX公司动态" | web-search 技能 |
| 两者兼有 | "XX技术的学术论文和商业应用" | 两个都用，分别查询 |

**判断依据**：
- 关键词含"论文""文献""研究""期刊""引用" → **学术路由**（见下方学术研究规则）
- 关键词含"发布""产品""新闻""评测""公司""价格""上线" → **Web 搜索路由**（读取 \`skills/scientific-skills/web-search/SKILL.md\` 并按指引操作）
- 不确定 → 优先 Web 搜索（覆盖面更广），必要时补充学术查询

## 学术研究与引用——强制规则

**这是最高优先级规则，违反等同于学术不端。** 当路由判断为学术查询时，执行以下规则：
...
\`\`\`
用户请求涉及产品/新闻/技术（路由判断为非学术查询）
    ↓
第一步：Read skills/scientific-skills/web-search/SKILL.md
    ↓
第二步：按照 web-search 指引，调用搜索脚本
    ↓
第三步：基于搜索结果综合回答，标注信息来源
\`\`\`
